...
export const AppContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchedUserUuidRef = useRef<string | null>(null);
  ...
  }, [setUser, updateInvite]);

  useEffect(() => {
    const currentUuid = session?.user?.uuid ?? null;
    ...
    if (fetchedUserUuidRef.current === currentUuid) {
      return;
    }

    fetchedUserUuidRef.current = currentUuid;
    fetchUserInfo();
  }, [session?.user?.uuid, fetchUserInfo, setUser]);
