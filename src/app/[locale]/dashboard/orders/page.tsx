...
export default async function OrdersPage() {
  ...
  if (!user_uuid) {
    return <Empty message={t("common.auth_required")} />;
  }
  ...
  const columns: TableColumn[] = [
    ...
    { name: "product_name", title: t("my_orders.table.product_name") },
    {
      ...
      callback: (item: any) => {
        const statusMap: Record<string, string> = {
          created: t("my_orders.table.status_created"),
          paid: t("my_orders.table.status_paid"),
          deleted: t("my_orders.table.status_deleted"),
        };
    ...
    },
    {
      ...
      callback: (item: any) => format(new Date(item.paid_at), "yyyy-MM-dd HH:mm:ss"),
    },
    {
      callback: async (item: any) => {
        if (!item.stripe_session_id || !item.stripe_session_id.startsWith("cs_")) {
          return "";
        }
        ...
        if (!sub_id && item.paid_detail) {
          try {
            ...
            console.log("parse paid_detail failed: ", error);
          }
        }
